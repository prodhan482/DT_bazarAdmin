...
import { addItem, getOrderProductsFromOrder } from "../customerService";
...
import {getSingleItems as getSingleOrder} from "../../../Order/OrderList/orderListService"

function Reorder({ onClose, onSuccess }) {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchDropDowns = async () => {
      try {
        const customerOrderData = await getSingleOrder(id)
        const customerOrderProduct = await getOrderProductsFromOrder(id)
        const productData = await getProduct();
        
        const customerData = await getSingleCustomer(customerOrderData[0].customer);
        const timeSlotData = await getTimeSlot();
        const paymentTypeData = await getPaymentType();
        const addressData = await getSingleCustomerAddresses(customerOrderData[0].customer);
        ...
        const preselectedProducts = customerOrderProduct.orderProducts.map((orderProduct) => {
          const product = productData.find((p) => p._id === orderProduct.product._id);
          return {
            ...product,
            quantity: orderProduct.quantity, // Add the quantity property
          };
        });
  
        // Extract quantities from the preselectedProducts
        const preselectedQuantities = preselectedProducts.map((product) => product.quantity || 0);
  
        // Set the preselected products and quantities to the state
        setSelectedProductsList(preselectedProducts);
        setQuantities(preselectedQuantities);
      } catch (error) {
  ...
  return (
    <CreateOrderPageLayout title="Create Order" onSubmit={handleSubmit}>
      {showTable && (
        <div>
          ...
          <Table>
            ...
            <button
              className="bg-red-700 hover:bg-red-500 text-white font-bold py-1 px-2 rounded w-[16%] "
              type="button"
              onClick={handleAddSelectedProducts}
            >
              Add all selected product
            </button>
          </Table>
      ...
      <div className="bg-green-700 hover:bg-green-600 text-white font-bold py-1 px-2 rounded w-[16%]">
        <button type="button" onClick={handleToggle}>
          Add Product
        </button>
      </div>
