...
import { editItem, getSingleItems } from "../pbcNumberService"

import { getItems as getCategory } from "../../../Products/Category/categoryService";


function EditPbcNumber({ pbcNumber, onClose, onEditSuccess }) {
  ...
  const [image, setImage] = useState(null);
  const [imagePreview, setImagePreview] = useState(null);
  const [link, setLink] = useState(pbcNumber.link);
  const [categories, setCategories] = useState([]);
  const [selectedCategory, setSelectedCategory] = useState(pbcNumber.selectedCategory);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await editItem(pbcNumber._id, formData);
  ...
  return (
    <EditFormLayout
      title={"Update Promo Card by Number"}
      onClose={onClose}
